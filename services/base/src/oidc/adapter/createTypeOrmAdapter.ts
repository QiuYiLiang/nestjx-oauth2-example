...
export function createTypeOrmAdapter(dataSource: DataSource): any {
  class TypeOrmAdapter {
    private model: Repository<Record<string, any>>

    constructor(private name) {
      try {
        this.model = dataSource.getRepository(models.get(name))
      } catch (error) {
        console.log(error)
      }
    }
